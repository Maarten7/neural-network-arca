...
def save_output(acc, cost, test=False):
    ...
    with open(PATH + 'data/results/%s/acc_%s.txt' % (title + '_v2', mode), 'a') as f:
        f.write(str(acc) + '\n')
    with open(PATH + 'data/results/%s/cost_%s.txt' % (title + '_v2', mode), 'a') as f:
...
#                feed_dict = {model.x: events[j: j + batch_size], model.y: labels[j: j + batch_size]} 
        ...
        save_path = saver.save(sess, PATH + "weights/%s.ckpt" % (title + '_v2'))
        ########################################################################
    return epoch_loss
...
def main():
    ...
    with tf.Session(config=config) as sess:
        try:
            ...
            saver.restore(sess, PATH + "weights/%s.ckpt" % (title + '_v2'))
        except:
